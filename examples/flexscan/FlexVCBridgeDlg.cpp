...
ON_MESSAGE(WM_FLEXSCAN, OnFlexScan)
ON_MESSAGE(WM_FLEXSCAN_COMPLETE, OnFlexScanComplete)
...
BOOL CFlexVCBridgeDlg::OnInitDialog()
{
	...
	g_oTwain.InitTwain(m_hWnd);
    CFlexCall::Init((void*)m_hWnd);
    CASWorkFlexScan* pFlexRun = new CASWorkFlexScan();
	//pFlexRun->m_pTwain = (void*)&g_oTwain;
...
BOOL CFlexVCBridgeDlg::PreTranslateMessage(MSG* pMsg) 
{
//<<<<<<<TWAIN>>>>>>>>
	if ( g_oTwain.ProcessMessage(*pMsg) == TRUE )
		return TRUE;
	else
		return FALSE;
...
LRESULT CFlexVCBridgeDlg::OnFlexScan(WPARAM wParam, LPARAM lParam)
{
	//g_oTwain.SelectSource();
	g_oTwain.Acquire(TWCPP_ANYCOUNT);
	return 0;
}

LRESULT CFlexVCBridgeDlg::OnFlexScanComplete(WPARAM wParam, LPARAM lParam)
{
	CFlexBridge* pBridge = (CFlexBridge*)CBridgeManager::GetBridgePtrFlashID(0);    
    CASObject oRootObj;
    //CDIB* pImg = (CDIB*)wParam;
	std::string* sStr = (std::string*)wParam;

    pBridge->Root(oRootObj);
    
    if ( oRootObj.GetInstance() )
    {
		oRootObj.Call("addPage", *sStr); 

		oRootObj.Call("getscanButton").Call("setenabled", true);       
		//oRootObj.Call("getScanImage").Call("setSource", "scan.png");       
