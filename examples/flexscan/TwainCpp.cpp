/* Based on code submitted by Rajiv Ramachandran to
 * codeproject.com. See
 * http://www.codeproject.com/KB/audio-video/twaintest.aspx. No
...
/* Change upto bin-debug to the path to your swf. */
#define FLEX_SCAN_IMG_PATH "C:\\Documents and Settings\\anirudhs\\My Documents\\Visual Studio 2008\\Projects\\FlexVC\\FlexScan\\scan"
...
void CTwain::GetIdentity()
{
	...
	m_AppId.Version.MinorNum = 703;
	m_AppId.Version.Language = TWLG_USA;
	m_AppId.Version.Country = TWCY_USA;
	lstrcpy (m_AppId.Version.Info, "FlexScan 0.1");
	//m_AppId.ProtocolMajor = TWON_PROTOCOLMAJOR;
	//m_AppId.ProtocolMinor = TWON_PROTOCOLMINOR;
	m_AppId.ProtocolMajor = 1;
	m_AppId.ProtocolMinor = 7;
	m_AppId.SupportedGroups = DG_IMAGE | DG_CONTROL;
	strcpy (m_AppId.Manufacturer, "Adobe");
	strcpy (m_AppId.ProductFamily, "Flex VC Bridge Samples");
	strcpy (m_AppId.ProductName, "FlexScan");
...
BOOL CTwain::Acquire(int numImages)
{
	...
	{
            //SetCapability(ICAP_XFERMECH,(TW_UINT16)TWSX_NATIVE,false);
...
void CTwain::CopyImage(HANDLE hMem,TW_IMAGEINFO& info)
{
    /* Warning: You are following the road to frustration. */
    CDIB* oDIB = new CDIB;
    oDIB->CreateFromHandle(hMem, info.BitsPerPixel);
    static int pageCnt = 49;
    std::string sImgPath = FLEX_SCAN_IMG_PATH;
    sImgPath += pageCnt++;
    std::string sPng = ".png";
    std::string sBmp = ".bmp";
    std::string sFinalImg = "";
    WCHAR wszImgPath[256];
    WCHAR wszImgPath1[256];
    sFinalImg += sImgPath;
    sFinalImg += sBmp;
    //sFinalImg = sFinalImg.substr(0, sImgPath.length()-1) + sFinalImg.substr(sImgPath.length(), sFinalImg.length());
    CString ostr = sFinalImg.c_str();
    oDIB->SaveDIB(ostr, CDIB::BMP);
    ...
    MultiByteToWideChar( CP_ACP, 0, sFinalImg.c_str(),
                         sFinalImg.length(), wszImgPath,   
                         sizeof(wszImgPath)/sizeof(wszImgPath[0]) );
    wszImgPath[sFinalImg.length()] = L'\0';
    Image*   image = new Image(wszImgPath);
...
//sFinalImg = sFinalImg.substr(0, sImgPath.length()-1) + sFinalImg.substr(sImgPath.length(), sFinalImg.length());
    MultiByteToWideChar( CP_ACP, 0, sFinalImg.c_str(),
                         sFinalImg.length(), wszImgPath1,   
                         sizeof(wszImgPath1)/sizeof(wszImgPath1[0]) );
    wszImgPath1[sFinalImg.length()] = L'\0';
    stat = image->Save(wszImgPath1, &encoderClsid, NULL);
    
    delete image;
    GdiplusShutdown(gdiplusToken);
    delete oDIB;
    std::string* sPtrString = new std::string;
    
    (*sPtrString) = "scan";
    (*sPtrString) += pageCnt - 1;
    (*sPtrString) += ".png";
    SendMessage(m_hMessageWnd, WM_FLEXSCAN_COMPLETE, (WPARAM)sPtrString, (LPARAM)0);
}
